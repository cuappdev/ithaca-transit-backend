...
class Kml {
  ...
  lineNumber: number;
  // The full placemark - in case we need it in
  // future revisions of the implementation
  placemark: string;
  // The placemark "prefix" - the LEADING portion of
  // this KML that does not contain coordinates
  placemarkPrefix: string;
  // The placemark "suffix" - the TRAILING tion of
  // this KML that does not contain coordinates
  ...
  constructor (lineNumber: number, placemark: string) {
    ...
    let locations = placemark.match(regex) || [];
    ...
    // the order in the KML is longitude,
    // latitude; the constructor takes latitude, longitude
  }
  ...
  placemarkFromStartEndStops (start: Stop, end: Stop) {
    ...
    let placemarkNew =
      this.placemarkPrefix + locationsString + this.placemarkSuffix;
    return placemarkNew;
  }
}
